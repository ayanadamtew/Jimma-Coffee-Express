...
if (isset($_POST['publish'])) {
	...
	if ($select_image->rowCount() > 0 and $image != '') {
		...
		$insert_post = $conn->prepare("INSERT INTO `product`(id, name, price, image, product_detail, status) VALUES (?,?,?,?,?,?)");
...
if (isset($_POST['draft'])) {
	...
	$select_image = $conn->prepare("SELECT * FROM `product` WHERE image = ? AND admin_id = ?");
